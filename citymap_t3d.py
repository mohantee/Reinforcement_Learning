"""
===============================================================================
CITYMAP NAVIGATION WITH T3D (Twin Delayed DDPG)
===============================================================================
Adapted from citymap_assignment.py and ERA1S25.ipynb
"""
...
SENSOR_ANGLE = 45   
SPEED = 5           
MAX_TURN = 30      # Max degrees per step (Action space [-30, 30])
...
# ==========================================
# 2. T3D IMPLEMENTATION (From ERA1S25.ipynb)
# ==========================================
...
class TD3(object):
  ...
  def train(self, replay_buffer, iterations, batch_size=256, discount=0.99, tau=0.005, policy_noise=0.2, noise_clip=0.5, policy_freq=2):
    ...
    # We allow training even for small number of iterations if buffer is large enough
...
class CarBrain:
    def __init__(self, map_image: QImage):
        ...
        self.action_dim = 1 # Continuous steering
        self.max_action = 1.0
    ...
    def step(self, action_scalar):
        # Action is scalar float [-1, 1]
        # Current angle update
        turn = action_scalar * MAX_TURN
        self.car_angle += turn
        ...
        if car_center_val < 0.4:
            ...
            if has_next:
                # Reached a waypoint, but not end of episode unless it's the last one
                # For simplicity in this implementation, we can treat reaching a waypoint as a good event 
                # but not necessarily 'done' unless it's the final target.
                # However, original code checks 'has_next'.
                # Let's say reaching intermediate targets isn't 'done', just rewarding.
                # Re-calculate distance to new target
            ...
            # Reward for getting closer / punishment for getting further
            # And reward for alignment
            reward += next_state[3] * 20 # Reward matching t3dd.py
        ...
        if self.episode_timesteps >= MAX_EPISODE_STEPS:
            done = True
...
# ==========================================
# 4. CUSTOM WIDGETS (VISUALS) - REUSED
# ==========================================
...
class NeuralNavApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("NeuralNav T3D: Twin Delayed DDPG")
        ...
        self.val_avg = QLabel("0.00")
        self.val_avg.setStyleSheet(f"color: #FFD700; font-weight: bold;") # Gold color for avg
        sf_layout.addWidget(QLabel("Avg Reward:"), 2,0)
        sf_layout.addWidget(self.val_avg, 2,1)
        ...
        # Log Device Info
        self.log(f"Process Device: {device}")
        if torch.cuda.is_available():
            self.log(f"GPU: {torch.cuda.get_device_name(0)}")
        else:
            self.log("WARNING: Running on CPU. Install PyTorch with CUDA support for better performance.")
    ...
    def game_loop(self):
        ...
        # 2. Select Action (T3D Logic)
        if self.brain.total_timesteps < START_TIMESTEPS:
             # Random action for exploration
             action = np.random.uniform(-1.0, 1.0, size=self.brain.action_dim)
        else:
             # Policy action + noise
             action = self.brain.policy.select_action(state)
             if EXPL_NOISE != 0:
                 action = (action + np.random.normal(0, EXPL_NOISE, size=self.brain.action_dim)).clip(-1.0, 1.0)
        
        # 3. Step
        next_state, reward, done = self.brain.step(action[0]) # Action is array of size 1
        
        # 4. Store
        done_bool = float(done)
        self.brain.replay_buffer.add((state, next_state, action, reward, done_bool))
        
        # 5. Train
        if self.brain.total_timesteps >= START_TIMESTEPS:
            self.brain.policy.train(self.brain.replay_buffer, 1, BATCH_SIZE, DISCOUNT, TAU, POLICY_NOISE, NOISE_CLIP, POLICY_FREQ)
        ...
        # Reset if done
        if done:
            final_score = self.brain.score
            self.brain.reset()
            self.chart.update_chart(final_score)
            self.val_rew.setText(f"{final_score:.1f}")
            
            # Calculate Average
            scores_len = len(self.chart.scores)
            if scores_len > 0:
                last_n = self.chart.scores[-min(scores_len, 50):]
                avg_val = sum(last_n) / len(last_n)
                self.val_avg.setText(f"{avg_val:.2f}")

            self.log(f"Episode {self.brain.episode_num} | Score: {final_score:.1f}")
        
        self.update_visuals()
        self.val_steps.setText(str(self.brain.total_timesteps))
    ...
    def update_visuals(self):
        ...
        for i, coord in enumerate(self.brain.sensor_coords):
            if i < len(self.sensor_items):
                ...
                # Simple check if sensor is on obstacle (dark) or road (light)
                # Map logic: high brightness = road, low = obstacle/wall
                # check_pixel returns 0.0 to 1.0 (brightness)
                # Let's say < 0.4 is obstacle
                val = self.brain.check_pixel(coord.x(), coord.y())
        ...
        for i, t_item in enumerate(self.target_items):
            ...
            if t_item.is_active != is_active:
                t_item.set_active(is_active)
